document.getElementById('link-page').addEventListener('click', function() {
    window.location.href = 'https://link-bio-alpha.vercel.app/'; 
});
document.getElementById('link_en').addEventListener('click', function() {
    window.location.href = 'https://link-bio-alpha.vercel.app/'; 
});
document.getElementById('code-monet').addEventListener('click', function() {
    window.location.href = 'https://codemonet-iago-souzas-projects.vercel.app/'; 
});
document.getElementById('code-monet-en').addEventListener('click', function() {
    window.location.href = 'https://codemonet-iago-souzas-projects.vercel.app/'; 
});
document.getElementById('logconnect').addEventListener('click', function() {
    window.location.href = 'https://logconnect.dev.br/'; 
});
document.getElementById('logconnect_en').addEventListener('click', function() {
    window.location.href = 'https://logconnect.dev.br/'; 
});
